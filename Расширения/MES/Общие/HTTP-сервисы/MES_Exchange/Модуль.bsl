...
Функция СоздатьСпецификацию(Данные)
	...
	ВыпускПроизвольнымиПорциями =  Шапка.Получить("ВыпускПроизвольнымиПорциями");
	Если НЕ ВыпускПроизвольнымиПорциями = Неопределено Тогда
		Если  НЕ ТипЗнч(ВыпускПроизвольнымиПорциями) = Тип("Булево") Тогда
			результат["ЕстьОшибки"] = Истина;
			результат["Ошибки"].Добавить("Не Булево ВыпускПроизвольнымиПорциями") ;
			Возврат результат;
		КонецЕсли;
		СпРО.ВыпускПроизвольнымиПорциями = ВыпускПроизвольнымиПорциями;
	КонецЕсли;

    РодительКод =  Шапка.Получить("РодительКод");
	Если НЕ РодительКод = Неопределено Тогда
		РодительСс = Справочники.РесурсныеСпецификации.НайтиПоКоду(РодительКод);
		Если РодительСс = Неопределено или РодительСс =  Справочники.РесурсныеСпецификации.ПустаяСсылка() Тогда
			результат["ЕстьОшибки"] = Истина;
			результат["Ошибки"].Добавить("Не найдена группа Код" + РодительКод + " в РесурсныеСпецификации") ;
			Возврат результат;
		КонецЕсли;
		СпРО.Родитель = РодительСс;
	КонецЕсли;


    ВариантПодбораВДокументы =  Шапка.Получить("ВариантПодбораВДокументы");
	Если НЕ ВариантПодбораВДокументы = Неопределено Тогда
		ВариантПодбораВДокументыСс =
		ПредопределенноеЗначение("Перечисление.ВариантыПодбораСпецификацииВДокументы." + ВариантПодбораВДокументы );
		Если ВариантПодбораВДокументыСс = Неопределено Тогда
			результат["ЕстьОшибки"] = Истина;
			результат["Ошибки"].Добавить("Не найдено ВариантыПодбораСпецификацииВДокументы " + ВариантПодбораВДокументы + " в Перечисление.ВариантыПодбораСпецификацииВДокументы") ;
			Возврат результат;
		КонецЕсли;
		СпРО.ВариантПодбораВДокументы = ВариантПодбораВДокументыСс;
	КонецЕсли;

	СпособРаспределенияЗатратНаВыходныеИзделия = Шапка.Получить("СпособРаспределенияЗатратНаВыходныеИзделия");
	Если НЕ СпособРаспределенияЗатратНаВыходныеИзделия = Неопределено Тогда
		СпособРаспределенияЗатратНаВыходныеИзделияСс =
		ПредопределенноеЗначение("Перечисление.СпособыРаспределенияЗатратНаВыходныеИзделия." + СпособРаспределенияЗатратНаВыходныеИзделия );
		Если СпособРаспределенияЗатратНаВыходныеИзделияСс = Неопределено Тогда
			результат["ЕстьОшибки"] = Истина;
			результат["Ошибки"].Добавить("Не найдено СпособРаспределенияЗатратНаВыходныеИзделия " + СпособРаспределенияЗатратНаВыходныеИзделия + " в Перечисление.СпособыРаспределенияЗатратНаВыходныеИзделия") ;
			Возврат результат;
		КонецЕсли;
		СпРО.СпособРаспределенияЗатратНаВыходныеИзделия = СпособРаспределенияЗатратНаВыходныеИзделияСс;
	КонецЕсли;
	...
	Если Шапка["Сохранять"] = Истина Тогда
		...
		ПодразделениеДиспетчер =  Шапка.Получить("ПодразделениеДиспетчер");
		Если НЕ ПодразделениеДиспетчер = Неопределено Тогда
			ПодразделениеДиспетчерОб =        Справочники.СтруктураПредприятия.НайтиПоНаименованию(ПодразделениеДиспетчер);
			Если ПодразделениеДиспетчерОб  = Неопределено Тогда
				результат["ЕстьОшибки"] = Истина;
				результат["Ошибки"].Добавить("Не найден ПодразделениеДиспетчерОб" +ПодразделениеДиспетчер + " в СтруктураПредприятия 1с") ;
				Возврат результат;
			КонецЕсли;
			...
			РегСпец.Значение = ПодразделениеДиспетчерОб;
	...
	Для каждого элт из ДанныеРес Цикл
		...
		соотв_Материалы = Новый Соответствие;
		массив_Материалы = Новый Массив;
		Если Тип(данныеЭтапа["Материалы"]) = Тип("Соответствие") Тогда
			соотв_Материалы = данныеЭтапа["Материалы"];
		КонецЕсли;
        Если Тип(данныеЭтапа["Материалы"]) = Тип("Массив") Тогда
			массив_Материалы = данныеЭтапа["Материалы"];
		КонецЕсли;
		...
		Для каждого киз Из соотв_Материалы Цикл
			...
			Если ТекНом =  Неопределено или ТекНом = Справочники.Номенклатура.ПустаяСсылка() Тогда
				результат["ЕстьОшибки"] = Истина;
			...
			Если Способ_получения_материала = Перечисления.СпособыПолученияМатериаловВСпецификации.ПроизвестиПоСпецификации Тогда
				ИсточникПолученияПолуфабриката = Справочники.РесурсныеСпецификации.НайтиПоКоду(соотв_данные_мат["ИсточникПолученияПолуфабриката"]);
				Если ИсточникПолученияПолуфабриката =  Неопределено или ИсточникПолученияПолуфабриката = Справочники.РесурсныеСпецификации.ПустаяСсылка() Тогда
					результат["ЕстьОшибки"] = Истина;
					результат["Ошибки"].Добавить("Не найден ИсточникПолученияПолуфабриката " + соотв_данные_мат["ИсточникПолученияПолуфабриката"] + " в РесурсныеСпецификации") ;
					Возврат результат;
				КонецЕсли;
				СтрМат.ИсточникПолученияПолуфабриката = ИсточникПолученияПолуфабриката;
			КонецЕсли;
		...
		КонецЦикла;
        Для каждого элем Из массив_Материалы Цикл
			СтрМат = СпРО.МатериалыИУслуги.Добавить();

		    код_мат = элем["Мат_код"];
			ТекНом = Справочники.Номенклатура.НайтиПоКоду(код_мат);
			Если ТекНом =  Неопределено или ТекНом = Справочники.Номенклатура.ПустаяСсылка() Тогда
				результат["ЕстьОшибки"] = Истина;
				результат["Ошибки"].Добавить("Не найден код " + код_мат + " в Номенклатура") ;
			Возврат результат;
			КонецЕсли;
		    Способ_получения_материала_стр = элем["Способы_получения_материала"];
		    Способ_получения_материала_мета = Метаданные.Перечисления.СпособыПолученияМатериаловВСпецификации.ЗначенияПеречисления.Найти(Способ_получения_материала_стр);
			Если Способ_получения_материала_мета =  Неопределено Тогда
				результат["ЕстьОшибки"] = Истина;
				результат["Ошибки"].Добавить("Не найден Перечисления " + Способ_получения_материала_стр + " в СпособыПолученияМатериаловВСпецификации") ;
			...
			Способ_получения_материала = Перечисления.СпособыПолученияМатериаловВСпецификации[Способ_получения_материала_стр];
			Если Способ_получения_материала = Перечисления.СпособыПолученияМатериаловВСпецификации.ПроизвестиПоСпецификации Тогда
				ИсточникПолученияПолуфабриката = Справочники.РесурсныеСпецификации.НайтиПоКоду(элем["ИсточникПолученияПолуфабриката"]);
				Если ИсточникПолученияПолуфабриката =  Неопределено или ИсточникПолученияПолуфабриката = Справочники.РесурсныеСпецификации.ПустаяСсылка() Тогда
					результат["ЕстьОшибки"] = Истина;
					результат["Ошибки"].Добавить("Не найден ИсточникПолученияПолуфабриката " + элем["ИсточникПолученияПолуфабриката"] + " в РесурсныеСпецификации") ;
					Возврат результат;
				КонецЕсли;
				СтрМат.ИсточникПолученияПолуфабриката = ИсточникПолученияПолуфабриката;
			КонецЕсли;
			Статья_калькуляции_стр = элем["Материалы_Статья_калькуляции"];
		    Статья_калькуляции = Справочники.СтатьиКалькуляции.НайтиПоНаименованию(Статья_калькуляции_стр);
			Если Статья_калькуляции =  Неопределено Тогда
				результат["ЕстьОшибки"] = Истина;
				результат["Ошибки"].Добавить("Не найден Статья_калькуляции " + Статья_калькуляции_стр + " в СтатьиКалькуляции") ;
...
&НаСервере
Функция ПроверитьНеИспользованиеРесурснойВЗП(КодРес)
		//{{КОНСТРУКТОР_ЗАПРОСА_С_ОБРАБОТКОЙ_РЕЗУЛЬТАТА
	// Данный фрагмент построен конструктором.
	// При повторном использовании конструктора, внесенные вручную изменения будут утеряны!!!
	...
	Запрос.Текст =
		...
		|	ЗаказНаПроизводство2_2Продукция.Ссылка.Статус КАК Статус
		|ИЗ
		|	Документ.ЗаказНаПроизводство2_2.Продукция КАК ЗаказНаПроизводство2_2Продукция
		|ГДЕ
		|	ЗаказНаПроизводство2_2Продукция.Спецификация.Код = &КодРес
...
Функция УдалитьСпецификацию(Данные)
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Если НЕ Доступно Тогда
			...
			Возврат результат;
		КонецЕсли;
		НеИспользуется = ПроверитьНеИспользованиеРесурснойВЗП(Код);
		Если НЕ НеИспользуется Тогда
			результат["ЕстьОшибки"] = Истина;
			результат["Ошибки"].Добавить("Спецификация " + Код + " используется в ЗП") ;
			Возврат результат;
		КонецЕсли;
